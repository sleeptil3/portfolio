...
export default function ProjectFeature({ category, title, description, hashtags, visit, frontCode, backCode, reverse, image }) {
	...
	useEffect(() => {
		switch (image) {
			case 'mercuryMarathon': { setImg(<img className="-my-10" src={mercuryMarathon} srcSet={`${mercuryMarathon2x} 2x, ${mercuryMarathon3x} 3x`} alt="" />); break; }
			case 'justGiveMeSpace': { setImg(<img className="-my-10" src={justGiveMeSpace} srcSet={`${justGiveMeSpace2x} 2x, ${justGiveMeSpace3x} 3x`} alt="" />); break; }
			case 'mirror': { setImg(<img src={mirror} srcSet={`${mirror2x} 2x, ${mirror3x} 3x`} alt="" />); break; }
			case 'codelockr': { setImg(<img src={codelockr} srcSet={`${codelockr2x} 2x, ${codelockr3x} 3x`} alt="" />); break; }
	...
	return (
		<div className="py-6 mx-auto sm:py-10 md:py-20 xl:py-20 md:-my-20 px-2 lg:px-0 w-12/12 lg:w-10/12 xl:w-9/12 2xl:w-8/12 text-white ">
			<div className={`relative flex flex-col-reverse sm:flex-row sm:space-x-10 justify-around xl:justify-between items-center ${reverse ? 'sm:flex-row-reverse' : null}`}>
				<div
					className="relative bottom-10 z-10 order-1 mt-10 sm:mt-0 sm:w-6/12 sm:-my-20"
					data-aos-once="true"
					data-aos-offset="100"
				...
				<div
					className="relative z-10 px-2 flex flex-col space-y-10 md:space-y-4 lg:space-y-5 2xl:space-y-6 sm:w-5/12"
					data-aos-once="true"
					data-aos-offset="100"
					data-aos={reverse ? "fade-left" : "fade-right"}
				>
