	...
	return (
		<div className="flex justify-center items-start sm:items-center flex-grow max-w-full">
			<div className="flex flex-col sm:flex-row sm:justify-center sm:items-center sm:px-10 py-10 px-5 h-full w-11/12 lg:w-10/12 xl:w-9/12 2xl:w-8/12 max-width">
				...
				{showForm ? (
					<form name="contact" onSubmit={handleSubmit} data-aos-once="true" data-aos="fade-up" className="flex flex-col relative px-4 sm:ml-20 mr-10 sm:pl-20 z-10 font-medium text-lg sm:text-xl md:text-2xl space-y-4">
						<div className="">
							<label className="font-semibold text-lg block">
								Your Name
								<input className="block border w-full border-black rounded-md py-1 px-2 text-sm focus:outline-none focus:ring-0" type="text" name="name" value={formData.name} onChange={handleChange} />
							</label>
						</div>
						<div>
							<label className="font-semibold text-lg block">
								Email
								<input className="block border w-full border-black rounded-md py-1 px-2 text-sm focus:outline-none focus:ring-0" type="text" name="email" value={formData.email} onChange={handleChange} />
							</label>
						</div>
						<div>
							<label className="font-semibold text-lg block">
								Message
								<textarea className="block h-24 w-full border border-black rounded-md py-1 px-2 text-sm focus:outline-none focus:ring-0" type="text" name="message" value={formData.message} onChange={handleChange} />
							</label>
						</div>
